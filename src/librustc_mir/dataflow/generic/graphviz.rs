...
pub trait StateFormatter<'tcx, A> where A: Analysis<'tcx> {
...
impl<T: Idx> SimpleDiff<T> {
    ...
    pub fn new(bits_per_block: usize) -> Self {
        SimpleDiff {
            prev_state: BitSet::new_empty(bits_per_block),
            prev_loc: Location::START,
        }
    }
}
...
const LIMIT_40_ALIGN_1: Option<LineBreak> = Some(LineBreak {
    sequence: "<br/> ",
    limit: 40,
});
